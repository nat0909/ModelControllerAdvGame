...
class BattleEngine():
    ...
    def enemy_turn(scenario: Scenario):
        ...
        spaces = scenario._spaces
        battle_log = []

        for enemy in scenario._enemies:
            abr = enemy._abr
            old_pos = positions[abr]
            new_pos, attack_type = enemy.perform_turn(positions, char_abr, spaces)
            scenario.update_position(abr, new_pos)
            # hit, dmg = BattleEngine.attack(scenario, enemy, attack_type, char_dex)
                        
            log = TurnLog(enemy._name, old_pos, new_pos)
            battle_log.append(log)
        return battle_log

    @staticmethod
    def attack(scenario: Scenario, creature: Creature, attack_type: str, opp_dex: int): # NOTE: not tested yet
        acc = 0
        dmg = 0
