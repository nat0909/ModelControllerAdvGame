...
class TestScenario:
    def test_constructor(self):
        scenario, character, enemies = SampleObjects.create_scenario()
        assert scenario._character == character
        assert scenario._enemies == enemies
        assert scenario._positions == {"MI": 1, "G1": 3}

    def test_damage_reduces_hp_without_defeat(self):
        scenario, _, enemies = SampleObjects.create_scenario()
        enemy = enemies[0]
    ...
    def test_damage_ends_scenario_when_character_defeated(self, monkeypatch):
        scenario, character, _ = SampleObjects.create_scenario()
        called = []
    ...
    def test_remove_enemy_does_not_end_scenario_when_others_remain(self, monkeypatch):
        enemy1 = SampleObjects.create_enemy(abr="G1")
        enemy2 = SampleObjects.create_enemy(abr="G2")
